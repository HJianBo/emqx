...
%%--------------------------------------------------------------------
-module(emqx_mgmt_tenant_api_subscription_SUITE).
...
t_subscription_api(Config) ->
    ...
    {ok, Client} = ClientFn(?CLIENTID, #{username => ?USERNAME, proto_ver => v5}),
    ...
    BadHeaders = [Auth, emqx_tenant_test_helpers:tenant_header(<<"bad.test.emqx.com">>)],
    {ok, ResponseEmpty} = emqx_mgmt_api_test_util:request_api(get, Path, BadHeaders),
    DataEmpty = emqx_json:decode(ResponseEmpty, [return_maps]),
    ?assertEqual(0, length(maps:get(<<"data">>, DataEmpty))),
    ...
    {ok, BadResponse} = emqx_mgmt_api_test_util:request_api(get, Path, QS, BadHeaders),
    BadData = emqx_json:decode(BadResponse, [return_maps]),
    BadSubscriptionsList = maps:get(<<"data">>, BadData),
    ?assertEqual(0, length(BadSubscriptionsList), BadSubscriptionsList),
    ...
    ?assertEqual(1, maps:get(<<"count">>, Meta2), DataTopic2),
