        ...
        {ok, NConnPkt, NChannel = #channel{clientinfo = ClientInfo}} ->
            ...
            case authenticate(?CONNECT_PACKET(NConnPkt), NChannel1) of
                {ok, Properties, NChannel2} ->
                    %% store will_msg once autheticate successfully
                    %% fix: https://github.com/emqx/emqx/issues/8886
                    process_connect(
                        Properties,
                        NChannel2#channel{will_msg = emqx_packet:will_msg(NConnPkt)}
                    );
...
terminate(Reason, Channel = #channel{will_msg = WillMsg}) ->
    %% since will_msg is set to undefiend as soon as it is published, if will_msg
    %% still exists when the session is terminated, it will be published immediately
    WillMsg =/= undefined andalso publish_will_msg(WillMsg),
